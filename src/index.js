...
Youtube.prototype.parseAddQueue = function(){
	...
	if(self.addQueue.length > 0){
		...
		self.addVideo(self.addQueue[0].uri).then(function(){videoDefer.resolve()}, function(e){videoDefer.resolve()});
		...
		return deferred.resolve({added: 0});
	}
...
Youtube.prototype.addToBrowseSources = function () {
  ...
  this.commandRouter.volumioAddToBrowseSources(data);	this.commandRouter.volumioAddToBrowseSources(data);
};
...
Youtube.prototype.clearAddPlayTrack = function(track){
	...
	var deferred = libQ.defer();
	ytdl.getInfo("https://youtube.com/watch?v=" + track.uri, { filter: "audioonly"}, function(err, info){
		if(err){
...
Youtube.prototype.prefetch = function(nextTrack){
	...
	ytdl.getInfo("https://youtube.com/watch?v=" + nextTrack.uri, { filter: "audioonly"}, function(err, info){
		if(err){
			console.log("Error opening Youtube stream, video is probably not valid.");
		}else {
			return self.mpdPlugin.sendMpdCommand('add "'+info["formats"][0]["url"]+'"',[])
...
Youtube.prototype.updateSettings = function(data) {
  ...
  if (resultsLimit <= 0) {
    var errorMsg = self.commandRouter.getI18nString('YOUTUBE_SETTINGS_UPDATE_FAILED_LIMIT_RANGE');
    self.commandRouter.pushToastMessage('failure', 'Saving settings failed', 'Results limit must be greater than 0 (zero).');
  } else {
    self.config.set('results_limit', resultsLimit);
    self.resultsLimit = resultsLimit;
    self.commandRouter.pushToastMessage('success', 'Settings saved', 'Settings successsfully updated.');
  }
